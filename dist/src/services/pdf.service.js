...
const C = {
    darkBlue: (0, pdf_lib_1.rgb)(0.102, 0.137, 0.494),
    midBlue: (0, pdf_lib_1.rgb)(0.157, 0.208, 0.576),
    gold: (0, pdf_lib_1.rgb)(0.784, 0.663, 0.318),
    deepRed: (0, pdf_lib_1.rgb)(0.718, 0.11, 0.11),
    lightGray: (0, pdf_lib_1.rgb)(0.98, 0.98, 0.98),
    midGray: (0, pdf_lib_1.rgb)(0.62, 0.62, 0.62),
    charcoal: (0, pdf_lib_1.rgb)(0.259, 0.259, 0.259),
...
async function generateUndertakingPDF(opts) {
    const { employeeName, designation = "CCE", companyName = "Woodrock Softonic Pvt Ltd", } = opts;
    ...
    const logoImg = await doc.embedJpg(Buffer.from(images_1.LOGO_B64, "base64"));
    const stampImg = await doc.embedJpg(Buffer.from(images_1.STAMP_B64, "base64"));
...
// ── Helpers ───────────────────────────────────────────────────────────────────
function wrapText(page, font, text, x, y, size, color, maxWidth = CW) {
...
// ── Header ────────────────────────────────────────────────────────────────────
function drawHeader(page, bold, regular, logo, companyName) {
    const BAR = 72;
    ...
    page.drawText("Fit Way Enclave, DN 12, Street 18, Salt Lake Sector 5, Kolkata - 700091", {
        x: tx, y: H - 44, size: 7.5, font: regular, color: (0, pdf_lib_1.rgb)(0.69, 0.745, 0.773),
    });
    page.drawText("contact@woodrockgroup.in  |  CIN: U74999WB2017PTC222743", {
        x: tx, y: H - 58, size: 7.5, font: regular, color: (0, pdf_lib_1.rgb)(0.69, 0.745, 0.773),
    });
}
// ── Footer ────────────────────────────────────────────────────────────────────
function drawFooter(page, regular) {
    ...
    page.drawText(foot, {
        ...
        size: 7, font: regular, color: (0, pdf_lib_1.rgb)(0.69, 0.745, 0.773),
    });
}
// ── Body ──────────────────────────────────────────────────────────────────────
function drawBody(page, bold, regular, name, designation, companyName) {
    let y = H - 98;
    // Title banner
    ...
    page.drawLine({
        ...
        thickness: 0.5, color: (0, pdf_lib_1.rgb)(0.88, 0.88, 0.88),
    });
    y -= 14;
    // Opening
    y = wrapText(page, regular, `I, ${name}, employed as ${designation} at ${companyName}, hereby acknowledge and agree to the following terms regarding my conduct and responsibilities while interacting with customers and handling company data:`, ML, y, 9.5, C.black);
    y -= 6;
    // Sections
    const sections = [
        {
...
// ── Signature section ─────────────────────────────────────────────────────────
function drawSignatureSection(page, bold, regular, stamp, name, designation, startY) {
    let y = startY - 8;
    page.drawLine({
        start: { x: ML, y }, end: { x: ML + CW, y },
        thickness: 0.5, color: (0, pdf_lib_1.rgb)(0.88, 0.88, 0.88),
