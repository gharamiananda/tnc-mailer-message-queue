...
const LINK_BLUE  = rgb(0.1,  0.2,  0.7 );
...
function wrap(
  ...
  x: number, y: number, size: number, color: any, maxW: number
): number {
  const lh = size * 1.55;
...
function underlineText(
  page: PDFPage, font: PDFFont, text: string,
  x: number, y: number, size: number, color: any
) {
...
async function drawHeader(
  ...
  doc: PDFDocument, logoUrl?: string
) {
...
function drawFooter(page: PDFPage, bold: PDFFont, regular: PDFFont) {
  page.drawLine({
...
function sectionHeader(
  page: PDFPage, bold: PDFFont, regular: PDFFont,
  num: string, title: string, y: number
): number {
  const numStr = `${num}. `;
...
function drawPage1(
  ...
  underlineText(page, bold,
    "EMPLOYEE CONDUCT UNDERTAKING & CONFIDENTIALITY DECLARATION",
    ML, y, 11, BLACK);
  y -= 30;
  ...
  // To block
  page.drawText("To,",     { x: ML, y,      size: 10, font: regular, color: DARK_GRAY }); y -= 18;
  page.drawText("The All", { x: ML, y,      size: 10, font: regular, color: DARK_GRAY }); y -= 18;
  page.drawText(companyName.toUpperCase(), { x: ML, y, size: 10, font: bold, color: DARK_GRAY }); y -= 18;
  y -= 4;

  // Subject
  page.drawText("Subject: ",
    { x: ML, y, size: 10, font: bold, color: BLACK });
  page.drawText("Undertaking for Professional Conduct and Data Confidentiality",
    { x: ML + bold.widthOfTextAtSize("Subject: ", 10), y, size: 10, font: regular, color: DARK_GRAY });
  y -= 28;
  ...
  y = wrap(page, regular,
    ...
    ML, y, 10, DARK_GRAY, CW);
  y -= 18;

  // ── Section 1 ──
  y = sectionHeader(page, bold, regular, "1", "Professional Conduct", y);
  y = wrap(page, regular,
    "  I understand that maintaining professionalism, politeness, and respect in all customer interactions is mandatory as part of my role.",
    ML, y, 10, DARK_GRAY, CW);
  y -= 18;
  ...
  y = wrap(page, regular,
    "  I strictly undertake that I will not engage in any of the following behaviours during calls, chats, emails, or any communication with customers:",
    ML, y, 10, DARK_GRAY, CW);
  y -= 10;
...
function drawPage2(page: PDFPage, bold: PDFFont, regular: PDFFont) {
  let y = H - 132;

  // ── Section 3 ──
  y = sectionHeader(page, bold, regular, "3", "Data Protection & Confidentiality", y);
  y = wrap(page, regular,
    "  I acknowledge that all customer data and company information is strictly confidential. I strictly agree that:",
    ML, y, 10, DARK_GRAY, CW);
  y -= 10;
  ...
  y = wrap(page, regular,
    "  I understand that any violation of the above terms will be treated as a serious breach of company policy and may result in strict action, including but not limited to:",
    ML, y, 10, DARK_GRAY, CW);
  y -= 10;
  ...
  y = wrap(page, regular,
    "If any employee fails to comply with the above, a penalty of Rs.50,000 may be imposed.",
    ML, y, 10, DARK_GRAY, CW);
  y -= 4;

  y = wrap(page, regular,
    "Additionally, the company reserves the right to recover any damages caused due to such actions.",
    ML, y, 10, DARK_GRAY, CW);
}
...
async function drawPage3(
  ...
  y = wrap(page, regular,
    "  I confirm that I have read, understood, and agreed to comply with this undertaking. I accept full responsibility for maintaining professional conduct and safeguarding all confidential information.",
    ML, y, 10, DARK_GRAY, CW);
  y -= 4;
  y = wrap(page, regular,
    "I am signing this document voluntarily and agree to abide by all the terms mentioned above.",
    ML, y, 10, DARK_GRAY, CW);
  y -= 20;
